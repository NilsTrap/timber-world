...
- **E1 + E2 DEPLOYED + SELF-TESTED ON STAGING (2026-06-16):** all four migrations pushed to staging (`fyzrtqsnmnizoxgcqsjc`); deals tables snapshotted (3 test rows → `/tmp`) before drop. Verified on the deployed staging portal: `/deals`→404; Settings intact; MCP tools/list has the 2 attribute tools + 5 deal tools, no `generate_document`; **`timber_create_deal` (name omitted) → ORD-070 / dealCode TIMSOM001 / name "Somms Ltd" / 1 line item** (the bug the review caught, now fixed); `list_deals` shows it; `status:"quoted"` → VALIDATION_ERROR; `allocate_deal_code` idempotent. Test deal deleted afterwards. **Not pushed to `main`/remote** (branch-local).

## What's NOT done yet (continue in this order — from the plan's task split)
1. **E3** — document data-assembly service (payload from order + line items + party company-card + totals + VAT + amount-in-words + allocated number) + **generation port** (interim local jsPDF adapter — the renderers are already salvaged at `features/orders/services/documents/*` — / Oscar-MCP adapter) + store in `order_documents` + bucket + **wire universal line-items + documents into the orders detail UI** (a new tab; UK-stairs tabs untouched). **Re-register the `timber_generate_document` MCP tool on orders** (it was de-registered in E2.4). Numbering helpers (`buildDocNumber`/`docNumberScope`/`allocateCounter`) are ready in `features/orders/services/numbering.ts`.
2. **E4** — rename "Users" → **"Orgs & People"**; add `crm_org_id`/`crm_synced_at` to `organisations`; extend create-org with the company card; `services/crm/oscarCrm.ts` (config-gated stub; Oscar `/api/crm-mcp`); thin cache + import picker; org MCP tools.
3. **E5** — complete the Timber MCP surface (every lifecycle step a tool) + **MCP-coverage check**.
4. **E1.3** — converge `ref_*` → views (independent; grep for external writers first).
5. **E6** — Oscar-side (gated on the Timber Oscar instance + Oscar dev agent): general doc-generation MCP tool, MCP registration, AI intake + doc-chasing workflows.

## Salvage status (deals → orders) — DONE
All model-agnostic services live under `features/orders/services/` (numbering, vat, dealModel, orderDeals) and the PDF **renderers** were moved to `features/orders/services/documents/` (amountInWords, specification, index, types). `features/deals` is **deleted**. Remaining: E3 builds the orders **document data-assembly + generation port + storage** on top of the salvaged renderers (the old deals `documentService` logic was deal-table-bound and is rebuilt, not copied). Optional/deferred: rename the **Orders** nav label to "Deals" (+ emerald grouping) — cosmetic, left as "Orders" for now.
