...
- **Commits done + verified (portal type-check green, 17 unit tests green):**
...
1. **E1.1+E1.2** — Settings nav (`/admin/settings/*`, `settings.view`) + move Fields/Packaging/Pricing-Units with redirects; `attributes.ts` service wrapping `catalog_fields`/`catalog_field_options` (+ MCP tools).
2. **E2.3+E2.4** — wire numbering into order create; **retire the deals feature** (salvage already done — the canonical services now live under `features/orders/services`): delete `features/deals`, `app/(portal)/deals`, drop `deals*` tables via a new migration (keep `deal_counters`/`next_counter`/`can_access_deal`); re-point/extend the MCP route off deals onto orders. Unify nav to one entry (recommend UI label **"Deals"** backed by `orders`; the emerald sidebar group already exists).
3. **E3** — document data-assembly service (payload from order + line items + party company-card + totals + VAT + amount-in-words + allocated number) + **generation port** (interim local jsPDF adapter — salvage `features/deals/services/documents/*` renderers — / Oscar-MCP adapter) + store in `order_documents` + bucket + **wire universal line-items + documents into the orders detail UI** (a new tab; UK-stairs tabs untouched).
4. **E4** — rename "Users" → **"Orgs & People"**; add `crm_org_id`/`crm_synced_at` to `organisations`; extend create-org with the company card; `services/crm/oscarCrm.ts` (config-gated stub; Oscar `/api/crm-mcp`); thin cache + import picker; org MCP tools.
5. **E5** — complete the Timber MCP surface (every lifecycle step a tool) + **MCP-coverage check**.
6. **E1.3** — converge `ref_*` → views (independent; grep for external writers first).
7. **E6** — Oscar-side (gated on the Timber Oscar instance + Oscar dev agent): general doc-generation MCP tool, MCP registration, AI intake + doc-chasing workflows.
