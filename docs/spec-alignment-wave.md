...
| K — CRM contacts + person-centric people management (2026-07-04): org_contacts entity on CRM cards, one obvious Add-person flow (existing-first + inline groups), platform People directory, membership-model reconciliation. Order K1→K3→K2→K4. NO MCP in this epic (later J extension). Runs PARALLEL with J (K owns counterparties/organisations/access COMPONENTS; J owns api/timber-mcp + service extractions — if both need features/access/actions, J3 owns it, K reads only). | `wnwgxb` | K1 `cznr26` · K3 `t3w6zn` · K2 `77cw34` · K4 `fabg8f` |
