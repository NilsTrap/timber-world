...
BEGIN
  ...
  INSERT INTO public.orders(id,code,name,customer_organisation_id,seller_organisation_id,buyer_organisation_id,deal_kind,product_group,currency,value_cents,status,lifecycle_stage,deal_code,spine_id,upstream_deal_id,notes,created_by)
  VALUES(v_leg,'RFQ-'||upper(substr(replace(v_leg::text,'-',''),1,10)),coalesce(o.name,'Project')||' - purchase',r.organization_id,c.organization_id,r.organization_id,'purchase_only',o.product_group,o.currency,c.quote_total_cents,'draft','draft',NULL,o.spine_id,NULL,'Awarded RFQ total: '||c.quote_total_cents||' cents',public.current_portal_user_id());
