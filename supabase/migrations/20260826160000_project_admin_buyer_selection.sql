-- Let platform administrators correct a Draft project's buyer to any active
-- customer. Ordinary users remain limited to the represented trader's book.
...
BEGIN
  ...
  IF p_buyer_id IS NOT NULL THEN
    ...
    IF NOT public.is_current_user_platform_admin()
      AND NOT EXISTS (SELECT 1 FROM public.organisation_trading_partners WHERE organisation_id=v_order.seller_organisation_id AND partner_organisation_id=p_buyer_id)
    THEN RAISE EXCEPTION 'Buyer is not a trading partner'; END IF;
