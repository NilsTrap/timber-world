...
### Sub-Module Pattern
Each module has fine-grained sub-modules. For Orders the live ones are `orders.view` (see Orders + the List) and the per-tab toggles `orders.tab.list` / `orders.tab.prices` / `orders.tab.sales` / `orders.tab.production` / `orders.tab.analytics`. A user who has `orders.tab.production` (but not full `orders.view`) may edit **only** the production-tab fields of an order — date loaded, planned date, production m³/material/finishing, production + wood-art invoices/payments — enforced server-side by the `PRODUCTION_EDIT_FIELDS` whitelist in `updateOrder` (so they can never touch pricing/customer). (The separate `orders.tab.production.edit` sub-module was removed 2026-06-09 — seeing the Production tab now grants production editing.)
