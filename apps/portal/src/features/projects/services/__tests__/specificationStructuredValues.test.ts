...
assert.match(tables, />EUR total</);
assert.match(tables, />Total EUR for the whole quotation</);
assert.match(tables, /key===lastLineKey/);
const rfqCard = readFileSync("src/features/projects/components/ProjectRfqCard.tsx", "utf8");
assert.match(rfqCard, /Create supplier quotation/);
assert.equal(tables.match(/!border-\[#b8c8bf\]/g)?.length, 5);
