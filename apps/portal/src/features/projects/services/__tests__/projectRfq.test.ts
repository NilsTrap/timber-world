...
import { detailedTotalFromUnitPrice, pricesFromQuotation, quotationEntries, quotationPricingRows, quotationTotalCents, unitPriceFromDetailedTotal } from "../projectQuotationRows";
...
assert.equal(detailedTotalFromUnitPrice("6", 9.39), "56.34");
assert.equal(unitPriceFromDetailedTotal("56.34", 9.39), "6");
assert.equal(unitPriceFromDetailedTotal("10", 0), "");
assert.deepEqual(pricesFromQuotation([{ targetType:"process", targetId:"p", label:"Powder coating", quantity:9.39, unit:"m²", unitPriceCents:600 }], "itemized"), { "process:p":"56.34" });
...
const buyerOfferMigration = readFileSync("../../supabase/migrations/20260904060000_buyer_offer_publish_decision.sql", "utf8");
...
assert.match(commercialAction, /buyerVisible=\["published","accepted","rejected"\]/);
...
assert.match(commercialCard, /const hasSupplierProposal = \(state\.sources\?\.length \?\? 0\) > 0 \|\| state\.lines\.length > 0/);
...
assert.match(buyerOfferMigration, /commercial_rollup_state IN \('draft','published','accepted','rejected','stale'\)/);
assert.match(buyerOfferMigration, /CREATE OR REPLACE FUNCTION public\.publish_project_commercial_offer/);
assert.match(buyerOfferMigration, /CREATE OR REPLACE FUNCTION public\.decide_project_commercial_offer/);
assert.match(buyerOfferMigration, /target\.buyer_organisation_id[\s\S]*current_user_in_org/);
assert.match(buyerOfferMigration, /p_decision='accepted'[\s\S]*lifecycle_stage='awarded'[\s\S]*source_order_id/);
assert.match(buyerOfferMigration, /commercial_buyer_notes=nullif\(trim\(p_notes\),''\)/);
assert.match(buyerOfferMigration, /lifecycle_stage='awaiting_buyer_decision'/);
assert.match(buyerOfferMigration, /commercial_rollup_state<>'published'.*OFFER_NOT_OPEN/);
assert.match(buyerOfferMigration, /commercial_rollup_state<>'draft'[\s\S]*commercial_stale_at IS NOT NULL[\s\S]*OFFER_NOT_READY/);
assert.match(buyerOfferMigration, /p_decision IS NULL OR p_decision NOT IN\('accepted','rejected'\)[\s\S]*char_length\(coalesce\(p_notes,''\)\)>4000/);
assert.match(rollupMigration, /UPDATE public\.orders SET commercial_rollup_state='stale',commercial_stale_at=now\(\)[\s\S]*project_leg_commercial_sources/);
assert.match(commercialCard, /if \(result\.data\.canDecide\) setOpen\(true\)/);
assert.match(buyerOfferMigration, /commercial_rollup_state='accepted'[\s\S]*commercial_decided_at=coalesce/);
assert.match(buyerOfferMigration, /project_commercial_lock_finalized_offer/);
assert.match(buyerOfferMigration, /OLD\.commercial_rollup_state='published'[\s\S]*OFFER_LOCKED/);
assert.match(buyerOfferMigration, /OLD\.commercial_rollup_state='accepted'[\s\S]*NEW\.commercial_rollup_state:='accepted'/);
assert.equal((buyerOfferMigration.match(/id=p_order_id AND deleted_at IS NULL FOR UPDATE/g) ?? []).length, 2);
assert.match(buyerOfferMigration, /get_project_commercial_sources\(target\.id\)[\s\S]*sourceVersion[\s\S]*sourceUpdatedAt/);
assert.match(commercialCard, /Offer notes \(optional\)/);
assert.match(commercialCard, /Decision notes \(optional\)/);
assert.match(commercialAction, /canViewPrivate=canBuild/);
assert.doesNotMatch(rfqCard, /<TraderMarginCard/);
...
assert.doesNotMatch(rfqCard, /How is this quotation priced\?/);
assert.match(specificationTables, /Clear the line and process prices before entering a total for the whole quotation/);
assert.match(specificationTables, /Unit price/);
assert.match(specificationTables, /EUR total/);
assert.match(specificationTables, /Total EUR for the whole quotation/);
assert.match(specificationTables, /Line total/);
...
assert.match(specificationTables, /disabled=\{!active\|\|quotation\.disabled\|\|quotation\.pending\|\|saveStatus==="saving"\|\|saveStatus==="error"\}/);
...
assert.ok(detailView.indexOf("<ProjectRfqCard") > detailView.indexOf("<ProjectSpecificationEditor"), "Supplier quotations render below Technical specification");
