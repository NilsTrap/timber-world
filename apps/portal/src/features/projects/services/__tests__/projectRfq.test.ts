...
assert.match(rfqCard, /Enter unit prices or process totals in the expanded specification below/);
assert.doesNotMatch(rfqCard, /How is this quotation priced\?/);
assert.match(rfqCard, /Clear the detailed line and process prices before entering one total for the whole quotation/);
assert.match(specificationTables, /Unit price/);
assert.match(specificationTables, /Process total/);
