...
export async function addProjectOfficialImage(projectId: string, fileId: string): Promise<ActionResult<{ position: number }>> {
  ...
  const { data: existingData } = await access.actor.db.from("order_files").select("id,thumbnail_sort_order")
  ...
  const used = new Set(existing.map((row)=>Number(row.thumbnail_sort_order)));
