...
import { quoteTotalToCents } from "../services/projectRfq";
...
export async function getEligibleProjectRfqCandidates(projectId: string): Promise<ActionResult<Array<{id:string;name:string}>>> {
  ...
  const {data,error}=await admin.from("organisations").select("id,name,is_active,is_trader,is_supplier,is_producer").eq("is_active",true).neq("id",owner.project.seller.id).order("name");
  if(error)return{success:false,error:"Could not load candidates",code:"FETCH_FAILED"};
  return{success:true,data:((data??[]) as Array<Record<string,unknown>>).filter((org)=>org.is_trader||org.is_supplier||org.is_producer).map((org)=>({id:org.id as string,name:org.name as string}))};
}
...
export async function requestProjectQuotations(raw: unknown): Promise<ActionResult<{id:string}>> {
  ...
  if(error)return{success:false,error:"Could not create quotation requests",code:"CREATE_FAILED"}; revalidatePath(`/projects/${parsed.data.projectId}`); return{success:true,data:{id:data as string}};
}
...
export async function awardProjectQuotation(raw: unknown): Promise<ActionResult<{projectId:string}>> {
  ...
  const owner=await requireOwner(parsed.data.projectId); if(!owner)return{success:false,error:"Not allowed",code:"FORBIDDEN"};
  const { data: rfq } = await owner.a.db.from("project_rfqs").select("id").eq("id",parsed.data.rfqId).eq("order_id",parsed.data.projectId).maybeSingle();
  if (!rfq) return { success:false,error:"Quotation request not found",code:"NOT_FOUND" };
  ...
  if(error)return{success:false,error:"Could not award quotation",code:"AWARD_FAILED"}; revalidatePath(`/projects/${parsed.data.projectId}`); return{success:true,data:{projectId:data as string}};
}
