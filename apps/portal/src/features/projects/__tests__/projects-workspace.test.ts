...
ok("catalogue process values are copied through one atomic price-free boundary", specificationActions.includes('rpc("create_project_specification_line_with_processes"') && specificationActions.includes('.eq("applies_to", "process")') && !processFieldsMigration.includes("unit_cost"));
