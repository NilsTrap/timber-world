...
ok("catalogue process values are copied into a separate price-free snapshot", specificationActions.includes('from("order_line_item_process_requirements").insert') && specificationActions.includes('.eq("applies_to", "process")') && !processFieldsMigration.includes("unit_cost"));
