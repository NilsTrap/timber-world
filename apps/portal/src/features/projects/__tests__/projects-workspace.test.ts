...
ok("ordinary payload has no sibling option key", projectLoader.includes('...(legOptions.length > 1 ? { legOptions } : {})') && !projectLoader.includes("createAdminClient"));
ok("leg selector navigates only to an existing project leg URL", legSelector.includes('router.push(`/projects/${id}`)') && legSelector.includes("value={currentProjectId}") && legSelector.includes("leg.id"));
...
ok("project name is primary and the header identifies spine and clicked leg", detail.includes("title={projectName || project.reference}") && detail.includes('`Spine ID: ${project.displaySpineCode}`') && detail.includes('`Leg: ${project.reference}`') && !detail.includes("<SummaryGrid") && !detail.includes('"Platform admin"'));
...
ok("process snapshot rendering has no pricing controls", specificationEditor.includes("line.processRequirements") && specificationEditor.includes("Processes:") && !specificationEditor.includes("requirement.unitCost"));
...
ok("HTML preview is sanitized, scriptless, and waits for images before capture", htmlViewer.includes("sanitizeProjectHtml") && htmlViewer.includes('sandbox="allow-same-origin"') && !htmlViewer.includes("allow-scripts") && htmlViewer.includes('import("html2canvas-pro")') && htmlViewer.includes("image.decode()") && htmlViewer.includes("imageTimeout: 30_000"));
...
ok("default image changes use the official-image manager gate and resequence the selected image first", officialImageActions.includes("setProjectOfficialImagePrimary") && officialImageActions.includes("mayManageOfficialImages(access, project)") && officialImageActions.includes("thumbnail_sort_order:index+1"));
...
ok("buyer removal uses a read visibility gate and narrowly scoped privileged designation update", officialImageActions.includes("requireVisibleProject(projectId, false)") && officialImageActions.includes("project.buyer_organisation_id === access.actor.orgId") && officialImageActions.includes("const mutationDb = createAdminClient()") && officialImageActions.includes("is_thumbnail:false,thumbnail_sort_order:null"));
