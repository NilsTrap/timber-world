...
const specificationStageEditMigration = readFileSync("../../supabase/migrations/20260905010000_allow_specification_stage_edits.sql", "utf8");
const traderSpecificationMigration = readFileSync("../../supabase/migrations/20260905150000_trader_specification_authoring.sql", "utf8");
...
ok("database specification mutation guards accept the configured Specification stage", specificationStageEditMigration.includes("lifecycle_stage NOT IN (''draft'',''specification'')") && specificationStageEditMigration.includes("create_project_specification_line_with_processes") && specificationStageEditMigration.includes("update_project_specification_structured_values") && specificationStageEditMigration.includes("update_project_specification_process_applicability"));
ok("trader specification mutations replace deal-term access with seller membership and trader persona", traderSpecificationMigration.includes("new_guard CONSTANT TEXT") && traderSpecificationMigration.includes("current_user_in_org(d.seller_organisation_id)") && traderSpecificationMigration.includes("o.is_active AND o.is_trader") && traderSpecificationMigration.includes("create_project_specification_line_with_processes") && traderSpecificationMigration.includes("update_project_specification_structured_values") && traderSpecificationMigration.includes("update_project_specification_process_applicability"));
ok("direct project parties use the full project view before RFQ candidate projection", projectLoader.includes("buyer_organisation_id === a.orgId || directPartyRow.seller_organisation_id === a.orgId") && projectLoader.indexOf("const actorIsDirectParty") < projectLoader.indexOf("loadRfqCandidateProject(a.db") && projectLoader.includes("!a.isPlatformAdmin && !actorIsDirectParty"));
ok("specification mutations verify seller-party ownership before trusted project loading", specificationActions.includes('select("seller_organisation_id")') && specificationActions.includes("party?.seller_organisation_id !== a.orgId") && specificationActions.includes("getOrderDeal(a.isPlatformAdmin ? a.db : admin"));
...
ok("RFQ candidate images expose viewing without mutation controls", projectLoader.includes("loadSpineProjectImages(projectAdmin, candidate.spineId)") && projectLoader.includes("isRfqCandidate: true"));
