...
ok("next-leg append remains separate from the two bilateral party cards", detail.includes("<ProjectNextLegControl") && nextLegControl.includes("setProjectSeller") && !parties.includes("setProjectSeller") && !nextLegControl.includes("PartySlot"));
ok("seller dropdowns group traders before suppliers and manufacturers", parties.indexOf("<SelectLabel>Traders</SelectLabel>") < parties.indexOf("<SelectLabel>Suppliers / Manufacturers</SelectLabel>") && nextLegControl.indexOf("<SelectLabel>Traders</SelectLabel>") < nextLegControl.indexOf("<SelectLabel>Suppliers / Manufacturers</SelectLabel>"));
...
ok("seller correction serializes spine mutations and keeps visible leg ids stable", legCorrectionMigration.includes("pg_advisory_xact_lock") && legCorrectionMigration.includes("trg_lock_project_spine_mutation") && !legCorrectionMigration.includes("next_counter") && !legCorrectionMigration.includes("UPDATE public.orders SET deal_code"));
