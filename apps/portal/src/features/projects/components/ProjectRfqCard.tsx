...
import { pricesFromQuotation, quotationEntries, quotationPricingRows, quotationTotalCents, type PricingRow, type ProjectQuotationPricingMode } from "../services/projectQuotationRows";
...
export function ProjectRfqCard({ projectId, currency, canManage, canEnterCandidateQuotation, initialOptions, lines }: {
  ...
  const [pricingMode,setPricingMode]=useState<ProjectQuotationPricingMode|null>(null);
  ...
  const load = useCallback(async () => {
    ...
    if (ownCandidate) {setNotes(ownCandidate.quoteNotes??"");setPrices(pricesFromQuotation(ownCandidate.quoteEntries));setPricingMode(ownCandidate.pricingMode);setProjectTotal(ownCandidate.pricingMode==="total"&&ownCandidate.quoteTotalCents!=null?(ownCandidate.quoteTotalCents/100).toFixed(2):"");}
    setLoaded(true);
  ...
  function submitQuotation() {
    ...
    const entries=pricingMode==="itemized"?quotationEntries(lines,prices):[];
    const totalCents=pricingMode==="total"?quotationTotalCents(projectTotal):null;
    startTransition(async () => {
      const payload={candidateId,pricingMode,entries,totalCents,notes};
  ...
  return <ProjectSectionCard>
    ...
    {(rfq?.ownCandidateId&&!rfq.canManage&&rfq.status==="open"||rfq?.canManage&&adminCandidateId) ? !rfq.canManage&&deadlinePassed
      ...
      : <QuotationEntryForm lines={lines} currency={currency} mode={pricingMode} onModeChange={(next)=>{if(pricingMode&&pricingMode!==next&&(Object.keys(prices).length>0||projectTotal)){if(!window.confirm("Changing pricing mode replaces the current quotation prices. Continue?"))return;setPrices({});setProjectTotal("");}setPricingMode(next)}} projectTotal={projectTotal} setProjectTotal={setProjectTotal} prices={prices} setPrices={setPrices} notes={notes} setNotes={setNotes} pending={pending} onSubmit={submitQuotation} onCancel={rfq.canManage?()=>setAdminCandidateId(null):undefined}/>
      : null}
...
function QuotationDetail({candidate,currency,onClose}:{candidate:ProjectRfqCandidate;currency:string;onClose?:()=>void}){const showEntries=candidate.pricingMode==="itemized"||(candidate.pricingMode===null&&candidate.quoteEntries.length>0);const modeLabel=candidate.pricingMode==="total"?"One total project price":candidate.pricingMode==="itemized"?"Price by line/process":"Legacy quotation — pricing mode not recorded";return <div className="space-y-3 rounded-md border p-3"><div className="flex items-start justify-between"><div><p className="font-medium">{candidate.organisationName} quotation</p><p className="text-xs text-muted-foreground">{modeLabel}{candidate.submittedAt?` · Submitted ${new Date(candidate.submittedAt).toLocaleString()}`:" · Not submitted"}{candidate.updatedAt?` · updated ${new Date(candidate.updatedAt).toLocaleString()}`:""}{candidate.quoteEnteredAsAdmin?" · corrected by admin":""}</p></div>{onClose?<Button size="sm" variant="ghost" onClick={onClose}>Close</Button>:null}</div>{showEntries?<div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Requirement</th><th className="p-2 text-right">Quantity</th><th className="p-2">Unit</th><th className="p-2 text-right">Unit price</th><th className="p-2 text-right">Subtotal</th></tr></thead><tbody>{candidate.quoteEntries.map((entry)=><tr key={`${entry.targetType}:${entry.targetId}`} className="border-b last:border-0"><td className="p-2">{entry.label}</td><td className="p-2 text-right">{entry.quantity}</td><td className="p-2">{entry.unit}</td><td className="p-2 text-right">{formatCents(entry.unitPriceCents,currency)}</td><td className="p-2 text-right">{formatCents(Math.round(entry.quantity*entry.unitPriceCents),currency)}</td></tr>)}</tbody></table></div>:candidate.pricingMode==="total"?<p className="rounded-md bg-muted p-3 text-sm">Total price for the complete project work package</p>:null}<p className="text-right font-semibold">Total: {formatCents(candidate.quoteTotalCents??0,currency)}</p>{candidate.quoteNotes?<p className="text-sm">{candidate.quoteNotes}</p>:null}{candidate.status==="awarded"?<p className="text-xs text-muted-foreground">Awarded quotation is locked for the supplier.</p>:null}</div>}
function QuotationEntryForm({lines,currency,mode,onModeChange,projectTotal,setProjectTotal,prices,setPrices,notes,setNotes,pending,onSubmit,onCancel}:{lines:ProjectLine[];currency:string;mode:ProjectQuotationPricingMode|null;onModeChange:(mode:ProjectQuotationPricingMode)=>void;projectTotal:string;setProjectTotal:(value:string)=>void;prices:Record<string,string>;setPrices:React.Dispatch<React.SetStateAction<Record<string,string>>>;notes:string;setNotes:(value:string)=>void;pending:boolean;onSubmit:()=>void;onCancel?:()=>void}){const rows=quotationPricingRows(lines);const rowCents=(row:PricingRow)=>Math.round(Number(prices[row.key]||0)*100*row.quantity);const totalCents=rows.reduce((sum,row)=>sum+rowCents(row),0);const totalValue=quotationTotalCents(projectTotal);return <div className="space-y-3 rounded-md border p-3"><fieldset className="flex flex-wrap gap-4"><legend className="mb-2 font-medium">How is this quotation priced?</legend><label className="flex items-center gap-2"><input type="radio" checked={mode==="itemized"} onChange={()=>onModeChange("itemized")}/>Price by line/process</label><label className="flex items-center gap-2"><input type="radio" checked={mode==="total"} onChange={()=>onModeChange("total")}/>One total project price</label></fieldset>{mode==="itemized"?<div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Requirement</th><th className="p-2 text-right">Quantity</th><th className="p-2">Unit</th><th className="p-2 text-right">Unit price ({currency})</th><th className="p-2 text-right">Total</th></tr></thead><tbody>{rows.map((row)=><tr key={row.key} className="border-b last:border-0"><td className="p-2">{row.label}</td><td className="p-2 text-right">{row.quantity}</td><td className="p-2">{row.unit}</td><td className="p-2"><Input aria-label={`Unit price for ${row.label}`} className="ml-auto w-32 text-right" type="number" min="0" step="0.01" value={prices[row.key]??""} onChange={(event)=>setPrices((current)=>({...current,[row.key]:event.target.value}))}/></td><td className="p-2 text-right">{(rowCents(row)/100).toFixed(2)}</td></tr>)}</tbody></table></div>:mode==="total"?<Field label={`Total project price (${currency})`}><Input aria-label="Total project price" type="number" min="0" step="0.01" value={projectTotal} onChange={(event)=>setProjectTotal(event.target.value)}/></Field>:<p className="text-sm text-muted-foreground">Choose a pricing mode to enter the quotation.</p>}<div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end"><Field label="Notes and exceptions"><Input value={notes} onChange={(event)=>setNotes(event.target.value)}/></Field><p className="pb-2 font-semibold">Total: {mode==="itemized"?(totalCents/100).toFixed(2):totalValue==null?"—":(totalValue/100).toFixed(2)} {currency}</p><div className="flex gap-2">{onCancel?<Button variant="outline" onClick={onCancel}>Cancel</Button>:null}<Button disabled={pending||!mode||(mode==="itemized"&&(rows.length===0||Object.keys(prices).length===0))||(mode==="total"&&totalValue===null)} onClick={onSubmit}>{pending?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:null}Submit quotation</Button></div></div></div>}
