...
import { pricesFromQuotation, quotationEntries, quotationPricingRows, type PricingRow } from "../services/projectQuotationRows";
...
export function ProjectRfqCard({ projectId, currency, canManage, canEnterCandidateQuotation, initialOptions, lines }: {
  ...
  const load = useCallback(async () => {
    ...
    if (ownCandidate) {setNotes(ownCandidate.quoteNotes??"");setPrices(pricesFromQuotation(ownCandidate.quoteEntries));}
    setLoaded(true);
  ...
  function submitQuotation() {
    ...
    const entries=quotationEntries(lines,prices);
    startTransition(async () => {
