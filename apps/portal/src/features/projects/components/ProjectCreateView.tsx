...
import { MAX_PROJECT_FILE_BYTES, normaliseProjectName, normaliseProjectPath, pathFromBrowserFile, projectPathKey, replacePathPrefix } from "../filePaths";
...
export function ProjectCreateView({ viewer }: { viewer: ProjectsViewer }) {
  ...
  const addFolder = () => {
    ...
    if (blocked) {
      setMessage(`${blocked}: a file already uses this path.`);
      return;
    }
  ...
  const addFiles = (incoming: File[]) => {
    ...
    for (const file of incoming) {
      ...
      if (file.size > MAX_PROJECT_FILE_BYTES) {
        errors.push(`${path.path}: over 100 MB`);
        continue;
      }
      ...
      if (ancestorKeys.some((ancestor) => existingFileKeys.has(ancestor))) {
  ...
  const renameFile = (item: StagedFile) => {
    ...
    if (files.some((other) => other.id !== item.id && projectPathKey(other.relativePath) === projectPathKey(target)) || manualFolders.some((folder) => projectPathKey(folder) === projectPathKey(target))) {
      setMessage("That name is already used in this folder.");
      return;
    }
  ...
  const renameFolder = (folder: string) => {
    ...
    const target = parent ? `${parent}/${valid}` : valid;
    if (files.some((item) => !item.relativePath.startsWith(`${folder}/`) && projectPathKey(item.relativePath) === projectPathKey(target)) || folders.some((path) => path !== folder && !path.startsWith(`${folder}/`) && projectPathKey(path) === projectPathKey(target))) {
      setMessage("That folder name is already used.");
      return;
    }
    ...
    if (new Set(nextPaths.map(projectPathKey)).size !== nextPaths.length) {
      setMessage("That folder name would create duplicate paths.");
      return;
    }
    setFiles((current) =>
      current.map((item) => ({
        ...item,
        relativePath: replacePathPrefix(item.relativePath, folder, target),
      })),
    );
  ...
  const submit = async () => {
    ...
    setCreating(false);
    if (failures === 0) {
      setHadSaveFailure(false);
      return;
    }
    setHadSaveFailure(true);
    setMessage(`${failures} item(s) failed. The project and successful items are saved; retry below.`);
  };

  const complete = !!created && files.every((file) => file.status === "done") && manualFolders.every((path) => createdFolderPaths.has(projectPathKey(path)));

  useEffect(() => {
    if (complete && created && !creating && !hadSaveFailure) router.replace(`/projects/${created.id}`);
  }, [complete, created, creating, hadSaveFailure, router]);
        ...
        <div className="rounded-lg border bg-card divide-y">
          {folders.map((folder) => (
            <div key={folder} className="flex items-center gap-2 px-3 py-2 text-sm">
              ...
              {!created ? (
                <Button type="button" variant="ghost" size="icon" aria-label={`Rename ${folder}`} onClick={() => renameFolder(folder)}>
                  <Pencil className="h-4 w-4" />
                </Button>
              ) : null}
              {!created ? (
                <Button type="button" variant="ghost" size="icon" aria-label={`Remove ${folder}`} onClick={() => removeFolder(folder)}>
        ...
        <div className="rounded-lg border bg-card divide-y">
          {files.map((item) => (
            <div key={item.id} className="flex items-center gap-3 p-3">
              ...
              {item.status === "failed" && created ? (
                <Button type="button" variant="outline" size="sm" onClick={() => uploadOne(created, item)}>
                  <RotateCcw className="mr-1 h-3.5 w-3.5" /> Retry
                </Button>
              ) : null}
              {item.kind === "file" && item.status !== "done" && item.status !== "uploading" ? (
                <Button type="button" variant="ghost" size="icon" aria-label="Rename file" onClick={() => renameFile(item)}>
                  <Pencil className="h-4 w-4" />
                </Button>
              ) : null}
              {item.status !== "done" && item.status !== "uploading" ? (
                <Button type="button" variant="ghost" size="icon" aria-label="Remove file" onClick={() => setFiles((current) => current.filter((row) => row.id !== item.id))}>
                  <Trash2 className="h-4 w-4" />
                </Button>
              ) : null}
            </div>
