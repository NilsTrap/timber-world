...
export function ProjectOfficialImages({ projectId, initialFiles, canManage, canRemove }: Props) {
  ...
  return <>
    <section className="overflow-hidden rounded-lg border bg-card">
      <button type="button" className="flex w-full items-center justify-between gap-3 p-4 text-left" aria-expanded={open} aria-controls="project-images-content" onClick={() => setOpen((current) => !current)}><span><span className="block text-xl font-semibold">Images</span><span className="block text-sm text-muted-foreground">{images.length > 0 ? `${images.length} of 3 project images` : "No images uploaded"}</span></span>{open ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}</button>
      {open ? <div id="project-images-content" className="space-y-3 border-t p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Up to three project images. The default image appears first and is used in the projects list.</p>{canManage ? <><input ref={input} className="hidden" type="file" accept="image/*" multiple onChange={(event) => void upload(event.target.files)} /><Button type="button" size="sm" disabled={busy || images.length >= 3} onClick={() => input.current?.click()}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />} Upload images</Button></> : null}</div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{[0, 1, 2].map((slot) => {
