/**
 * Deal & document numbering — Nils's confirmed conventions.
 *
 * Pure formatters (unit-tested) + an atomic allocator that calls the SQL
 * `next_counter(scope)` function. Purchase and sale series are deliberately
 * separate (Nils: "purchase codes and sales codes are not the same").
 */
import type { DbClient, DocType } from "../types";
...
/** Counter scope for a deal code (per entity + client). */
...
export interface DocNumberParams {
  docType: DocType;
  /** Seller/entity code (TIM, TWG) — drives invoice/contract series. */
  entityCode: string;
  /** Issue date (ISO or Date) — drives date-based series (S-/P-YYYYMMDD). */
  date: string | Date;
  /** Allocated sequence value for the relevant scope. */
  seq: number;
}

/**
 * Build a document number for a given type. Sale series use S-/TWG-style;
 * purchase series use P-style. Sequence is allocated separately per scope.
 */
export function buildDocNumber(p: DocNumberParams): string {
  ...
  switch (p.docType) {
    ...
    case "contract":
      // Sales contract: S-YYYYMMDD, with -n if multiple the same day.
...
/** Counter scope for a document number. Date-based series include the date. */
export function docNumberScope(docType: DocType, entityCode: string, date: string | Date, dealId?: string): string {
  const entity = entityCode.toUpperCase();
  switch (docType) {
    case "sales_spec":
    case "purchase_spec":
      // Specs are sequential per deal.
      return `doc:${docType}:deal:${dealId ?? "unknown"}`;
    case "contract":
...
export async function allocateCounter(db: DbClient, scope: string): Promise<number> {
  ...
  const { data, error } = await (db as any).rpc("next_counter", { p_scope: scope });
  if (error) {
    throw new Error(`Counter allocation failed for ${scope}: ${error.message}`);
  }
  const value = typeof data === "number" ? data : Number(data);
  if (!Number.isFinite(value)) {
    throw new Error(`Counter allocation returned non-numeric value for ${scope}`);
  }
  return value;
}
