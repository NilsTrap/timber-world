...
export async function createOrder(input: {
  ...
  // 5. Determine party slots (role-based auto-fill)
  // - Admin: free choice — both come from input.
  // - Non-admin manufacturer: own org forced as the Manufacturer (seller); user
  //   picks the Customer.
  // - Non-admin customer-side: own org forced as the Customer; user picks the
  //   Manufacturer (seller).
  const userOrgId = session.currentOrganizationId || session.organisationId;
  let customerOrgId: string | null = inputCustomerOrgId;
  let sellerOrgId: string | null = inputSellerOrgId;
  ...
  if (!isAdmin(session)) {
    ...
    if (userIsManufacturer) {
      ...
      customerOrgId = inputCustomerOrgId;
    } else {
