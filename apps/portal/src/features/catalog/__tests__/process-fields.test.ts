...
assert.match(migration, /line\.origin_line_item_id IS NULL/);
assert.match(migration, /btrim\(value\)<>''/);
assert.match(migration, /REQUIRED_PROCESS_VALUE_MISSING/);
assert.match(migration, /root_order_line_item_id/);
...
assert.throws(() => buildProcessRequirements(
  [{ is_required: true, catalog_fields: { field_key: "cutting", field_label: "Cutting" } }],
  {},
), /Missing required process field: Cutting/);
...
assert.match(specificationActions, /is_required/);
