...
export async function getAllOrgShipments(): Promise<ActionResult<OrgShipmentListItem[]>> {
  ...
  // Count packages for outgoing shipments by shipment_id OR source_shipment_id
  const outgoingIds = (outgoingData ?? []).map((s: { id: string }) => s.id);
  const outgoingPackageCounts: Map<string, { count: number; volume: number }> = new Map();

  if (outgoingIds.length > 0) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: outPkgs } = await (supabase as any)
    ...
    const outPkgIds = (outPkgs ?? []).map((p: { id: string }) => p.id);
    const outDeductions = new Map<string, number>();
    if (outPkgIds.length > 0) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: inputsData } = await (supabase as any)
        .from("portal_production_inputs")
        .select("package_id, volume_m3")
        .in("package_id", outPkgIds);

      for (const input of (inputsData ?? []) as { package_id: string; volume_m3: number | null }[]) {
        outDeductions.set(input.package_id, (outDeductions.get(input.package_id) ?? 0) + (Number(input.volume_m3) || 0));
      }
    }

    for (const pkg of outPkgs ?? []) {
      const currentVol = pkg.volume_m3 != null ? Number(pkg.volume_m3) : 0;
      const originalVol = currentVol + (outDeductions.get(pkg.id) ?? 0);
      if (pkg.shipment_id && outgoingIds.includes(pkg.shipment_id)) {
        const current = outgoingPackageCounts.get(pkg.shipment_id) ?? { count: 0, volume: 0 };
        current.count++;
        current.volume += originalVol;
        outgoingPackageCounts.set(pkg.shipment_id, current);
      }
      if (pkg.source_shipment_id && outgoingIds.includes(pkg.source_shipment_id) && pkg.source_shipment_id !== pkg.shipment_id) {
        const current = outgoingPackageCounts.get(pkg.source_shipment_id) ?? { count: 0, volume: 0 };
        current.count++;
        current.volume += originalVol;
        outgoingPackageCounts.set(pkg.source_shipment_id, current);
      }
    }
  }
  ...
  if (incomingIds.length > 0) {
    ...
    // Recover original volumes for consumed/partially-consumed packages
    const inPkgIds = (packages ?? []).map((p: { id: string }) => p.id);
    const inDeductions = new Map<string, number>();
    if (inPkgIds.length > 0) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: inputsData } = await (supabase as any)
        .from("portal_production_inputs")
        .select("package_id, volume_m3")
        .in("package_id", inPkgIds);

      for (const input of (inputsData ?? []) as { package_id: string; volume_m3: number | null }[]) {
        inDeductions.set(input.package_id, (inDeductions.get(input.package_id) ?? 0) + (Number(input.volume_m3) || 0));
      }
    }
    ...
    for (const pkg of packages ?? []) {
      const currentVol = pkg.volume_m3 != null ? Number(pkg.volume_m3) : 0;
      const originalVol = currentVol + (inDeductions.get(pkg.id) ?? 0);
  ...
  // Transform outgoing shipments (use counts from separate query)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const mapOutgoingRow = (row: any): OrgShipmentListItem => {
    const pkgData = outgoingPackageCounts.get(row.id) ?? { count: 0, volume: 0 };
    return {
      id: row.id,
