...
assert.ok(action.includes("NEUTRAL_MESSAGE") && action.includes("status === 404") && action.includes("status === 422"), "unknown accounts receive neutral confirmation");
...
assert.ok(form.includes("tokenProcessingStarted.current"), "the one-time recovery code is exchanged only once under React strict mode");
