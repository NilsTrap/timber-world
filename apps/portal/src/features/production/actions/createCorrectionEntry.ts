 ...
 * The correction inherits the same process_id and organisation_id, uses today's date.
 * Only standard (non-correction) validated entries can be corrected.
 *
 * Multi-tenancy:
 * - Organisation users can only create corrections for their own organisation's entries
 * - Super Admin can create corrections for any organisation's entries
 * - Correction inherits organisation_id from the original entry
 */
export async function createCorrectionEntry(
  ...
  // Fetch original entry — verify it exists, is validated, is 'standard' type, and owned by user's organisation
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: original, error: fetchError } = await (supabase as any)
    .from("portal_production_entries")
    .select("id, process_id, entry_type, status, organisation_id")
  ...
  // Organisation users can only correct their own organisation's entries
  if (isOrganisationUser(session) && original.organisation_id !== session.organisationId) {
    return { success: false, error: "Permission denied", code: "FORBIDDEN" };
  }
  // Super Admin can correct any entry
  ...
  // Insert new correction entry - inherits organisation_id from original
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
