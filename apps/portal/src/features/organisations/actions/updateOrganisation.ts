...
export async function updateOrganisation(
  ...
  const { data, error } = await (supabase as any)
    ...
    .eq("id", id)
    .select("id, code, name, is_active, is_external, is_customer, is_manufacturer, is_producer, is_supplier, is_trader, legal_address, vat_number, registration_number, country, phone, email, website, bank_name, bank_account_number, bank_swift_code, logo_url, crm_org_id, created_at, updated_at")
    .single();
  ...
  const organisation: Organisation = {
    ...
    isTrader: (data.is_trader as boolean) ?? false,
