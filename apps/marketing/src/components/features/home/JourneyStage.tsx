...
type JourneyStageProps = {
  /** Stage number (1-8) for accessibility */
  stageNumber: number;
...
export function JourneyStage({
  ...
  return (
    <div
      ...
      className="stack-card h-screen w-full overflow-hidden"
    >
      ...
      {/* Gradient Overlay - only when NOT showing gallery (gallery has its own) */}
      {!showGallery && (
        <div
      ...
      {/* Content text - at bottom - only when NOT showing gallery (gallery handles text) */}
      {!showGallery && (
        <div
