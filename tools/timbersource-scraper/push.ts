...
/**
 * Map scraper species/quality to stock_prices species/quality.
 * All oak species → "Oak". BC quality → Rustic.
 */
...
async function main() {
  ...
  for (const p of products) {
    ...
    if (stockM3) {
      ...
      // price_diff_percent: how TIM compares to competitor (in EUR)
      // Competitor price is in GBP, TIM price is in EUR
      // We store the diff based on EUR comparison (GBP * GBP_TO_EUR vs TIM EUR)
